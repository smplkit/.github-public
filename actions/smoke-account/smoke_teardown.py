...
bootstrap stashes ACCOUNT_TOKEN; we DELETE
/api/v1/accounts/current?purge=true with that token, which permanently
and irreversibly erases the account and all of its data across every
service — no soft-delete tombstone is left behind. 404 is treated as
success (the account is already gone — equivalent to a successful
purge).
"""
...
def main() -> int:
    ...
    url = f"{args.base_url}/api/v1/accounts/current?purge=true"
